...
```bash
docker compose up -d                 # 默认: 前端 8080 + 后端 API 3000
# 或指定端口(两个都可覆盖):
FRONTEND_PORT=8081 API_PORT=3002 docker compose up -d
```

- 首次启动：`vrcn-keygen` 自动生成主密钥 → 后端带加密启动(healthy) → 前端上线。
- 访问：页面 `http://<主机>:8080`（`FRONTEND_PORT` 可覆盖）；后端 API 默认也暴露在 `:3000`（`API_PORT` 可覆盖）。
- **连接方式（二选一，令牌相同）**：
  - **A 同源反代（推荐，单端口）**：门禁"后端地址"留自动预填（= 你打开页面用的那个地址）。
  - **B 直连后端（备用）**：门禁"后端地址"填 `http://<主机>:3000`（`API_PORT` 的值）。后端已开 CORS，跨端口直连可用。
...
- 前端与后端同 compose 网络。对外端口：前端 `8080`（`FRONTEND_PORT`）+ 后端 API `3000`（`API_PORT`）；后端默认也暴露，供浏览器直连 / 调试。
...
curl http://127.0.0.1:8080/api/config   # 同源反代路径(A) -> {"ok":true,...}
curl http://127.0.0.1:3000/api/config   # 直连后端路径(B) -> {"ok":true,...}
```
