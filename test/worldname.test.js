'use strict';
// 世界名查询模块: 缓存 / TTL / 合并 / 负缓存 / 重试 / 失败沿用旧名字 / 调度。
// 全部用注入的假 fetchWorld + 假时钟, 不发真网络; sleep 立即返回, 保证测试不会卡住。
const test = require('node:test');
const assert = require('node:assert');
const { createDb } = require('../src/db');
const { createWorldName } = require('../src/worldname');
...
test('worldname: peek 不发请求, 过期也照样返回', async () => {
  const t = harness();
...
test('worldname: 缓存新鲜时 0 次请求', async () => {
  const t = harness();
...
test('worldname: 缓存过期时重查并更新', async () => {
  const t = harness();
...
test('worldname: 无缓存时查一次并写入, 同时推 SSE 事件', async () => {
  const t = harness();
...
test('worldname: 空 id 直接返回 null 且不发请求', async () => {
  const t = harness();
...
test('worldname: 同一个 id 并发 5 次只发 1 次请求', async () => {
  const t = harness();
  let release;
...
test('worldname: 请求耗时超过 10 秒窗口, 期间后来者仍搭车不重发', async () => {
  const t = harness();
  let release;
...
test('worldname: 成功后缓存新鲜, 再要不会重复请求', async () => {
  const t = harness();
...
test('worldname: 404 进负缓存 15 分钟, 期间不发请求; 到点后允许重查', async () => {
  const t = harness();
...
test('worldname: 403 与 404 同样处理', async () => {
  const t = harness();
...
test('worldname: 404 且本地有旧名字时沿用旧名字并记日志', async () => {
  const t = harness();
...
test('worldname: 404 且无旧名字时返回 null, 日志用另一条文案', async () => {
  const t = harness();
  t.setImpl(failWith(404));
...
test('worldname: 网络错误就地重试 1 次后成功', async () => {
  const t = harness();
  let n = 0;
...
test('worldname: 5xx 也走就地重试', async () => {
  const t = harness();
  let n = 0;
...
test('worldname: 网络错误重试仍失败 → 负缓存 1 分钟 + 沿用旧名字', async () => {
  const t = harness();
  t.db.upsertWorldCache('wrld_a', '旧名字', t.state.t - 10 * HOUR);
...
test('worldname: 网络错误且无旧名字 → 返回 null', async () => {
  const t = harness();
  t.setImpl(failWith(-1));
  assert.equal(await t.wn.get('wrld_a'), null);
...
test('worldname: 429 走指数退避重试(5s 起步翻倍, 封顶 1h)', async () => {
...
test('worldname: 429 退避带上抖动', async () => {
  const t = harness();
  t.setImpl(failWith(429));
  await t.wn.get('wrld_rate');
...
test('worldname: fetchWorld 抛任意异常都不会让 get 拒绝', async () => {
  const t = harness();
...
test('worldname: 返回体没有 name 字段按失败处理', async () => {
  const t = harness();
  t.db.upsertWorldCache('wrld_a', '旧名字', t.state.t - 10 * HOUR);
...
test('worldname: bus 订阅者抛异常不影响查询结果', async () => {
  const db = createDb(':memory:');
  const wn = createWorldName({
    db,
...
test('worldname: 每分钟速率上限生效', async () => {
